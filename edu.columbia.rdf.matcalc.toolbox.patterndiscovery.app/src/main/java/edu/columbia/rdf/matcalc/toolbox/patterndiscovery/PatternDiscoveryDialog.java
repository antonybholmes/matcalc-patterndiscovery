...
public class PatternDiscoveryDialog extends ModernDialogHelpWindow implements ModernClickListener {
	 ...
	 * The delta field.
	 */
	private ModernCompactSpinner mDeltaField = new ModernCompactSpinner(0, 
					1, 
	 ...
	 * The support1 field.
	 */
	private ModernCompactSpinner mPhenField = 
			new ModernCompactSpinner(1, 100, 10);
	
	/**
	 * The support2 field.
	 */
	private ModernCompactSpinner mControlField = 
			new ModernCompactSpinner(1, 100, 10);
	
	/** The m genes field. */
	private ModernCompactSpinner mGenesField = 
			new ModernCompactSpinner(2, 
					1000000, 
					SettingsService.getInstance().getAsDouble("pattern-discovery.min-genes"));
	
	/** The m check phen min sup only. */
	private ModernTwoStateWidget mCheckPhenMinSupOnly =
			new ModernCheckSwitch("only");
	
	/** The m check con min sup only. */
	private ModernTwoStateWidget mCheckConMinSupOnly =
			new ModernCheckSwitch("only");
	 ...
	 * The check reset.
	 */
	private ModernTwoStateWidget mCheckReset = 
	...
	private class Group2ModernClickEvent implements ModernClickListener {
		...
		@Override
		public void clicked(ModernClickEvent e) {
			int n = MatrixGroup.findColumnIndices(mMatrix, 
					getControlGroup()).size();
	...
	public PatternDiscoveryDialog(ModernWindow parent, 
		...
		mZScoreField = new ModernCompactSpinner(0, 
						1000, 
